...
**What a sale would pay.** Any token with a pool gets priced: Uniswap V3 and its forks from the price and the liquidity in the current tick, Uniswap V4 the same way but found from the singleton's own `Initialize` logs, since V4 has no factory to ask, Uniswap V2 and its forks by exact constant product, Solidly forks such as Aerodrome. A Solidly stable pool uses an invariant this does not model and is reported unpriced rather than guessed at. On Solana a pump.fun token that has not graduated is priced exactly off its bonding curve, and its pools are found by walking the mint's largest accounts to a vault. `bouncer exit` and `bouncer wallet` answer for any token, not only for a launch.

**Can they pull the liquidity out from under you.** A pool is not money the token owes anybody: whoever holds the liquidity can take it back, and then there is nothing to sell into. So the slip reads who holds it. A V2 or Solidly pool by its LP token's balance sheet — burned, in a locker, or in somebody's hands. A V3 pool by tracing each position through the manager to whoever holds its NFT. Burned is certain; "locked" is deliberately a short list, and anything not on it is reported as withdrawable by address rather than called safe for having bytecode.
