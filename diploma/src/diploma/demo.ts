/**
 * Synthetic chain for `--demo`: four launches, two graduations (one a
 * 4-minute dev-funded sprint, one a slow class of 40 buyers), one curve
 * still filling, one dropout. Served by a fake fetch. Every address is
 * invented and the output is labelled DEMO. It never touches a network.
 */
...
export interface DemoToken {
  ...
  real: bigint;
  tokenReserve: bigint;
  /** Buys on the curve: [block offset from launch, buyer, quoteIn wei]. */
  buys: [number, string, bigint][];
...
export const DEMO = {
  ...
  tokens: {
    sprint: { token: "0x00c0ffee0000000000000000000000000000600d", curve: "0x0000c0a70000000000000000000000000000600d", deployer: DEV_A, name: "Sprint", symbol: "SPRINT", launched: HEAD - 2_600, swept: HEAD - 2_600 + 2_120, graduated: HEAD - 2_600 + 2_121, raised: 42n * 10n ** 17n, supplyToPool: 2n * 10n ** 26n, positionId: 4663n, taxBps: 300n, real: 42n * 10n ** 17n, tokenReserve: 0n, buys: sprintBuys, sells: [] } as DemoToken,
...
function curveLogs(t: DemoToken, from: number, to: number) {
  ...
  const logs: unknown[] = [];
  let index = 0;
  for (const [offset, who, quoteIn] of t.buys) {
    const b = t.launched + offset;
    if (b < from || b > to) continue;
    const fee = quoteIn / 100n;
    const tax = (quoteIn * t.taxBps) / 10_000n;
  ...
  return (async (_url: string | URL | Request, init?: RequestInit) => {
    ...
    const responses = requests.map((request) => {
      ...
      switch (request.method) {
        ...
        case "eth_getCode": return ok("0x6080demo");
        ...
        case "eth_getLogs": {
          const f = (request.params as [{ address?: string; fromBlock: string; toBlock: string }])[0];
          const from = Number(BigInt(f.fromBlock));
          const to = Number(BigInt(f.toBlock));
          if (!f.address || f.address === PONS_V2_FACTORY) return ok(factoryLogs(from, to));
          const t = byCurve.get(f.address.toLowerCase());
          return ok(t ? curveLogs(t, from, to) : []);
        }
        case "eth_call": {
